...
vars = load './jenkinsci/vars.groovy'

def javaBindingsLinux(String packageName, int parallelism, List environment) {
  withEnv(environment) {
    currentPath = sh(script: "pwd", returnStdout: true).trim()
    // do not use preinstalled libed25519
    sh """
      rm -rf /usr/local/include/ed25519*; \
      unlink /usr/local/lib/libed25519.so; \
      rm -f /usr/local/lib/libed25519.so.1.2.2
    """
    build.cmakeConfigure('build', "-DCMAKE_BUILD_TYPE=Release -DSWIG_JAVA=ON -DSWIG_JAVA_PKG=${packageName}")
    build.cmakeBuild('build', '--target irohajava', parallelism)
    artifactsPath = sprintf('%1$s/java-bindings-%2$s-%3$s-%4$s-%5$s.zip',
      [currentPath, buildType, os, sh(script: 'date "+%Y%m%d"', returnStdout: true).trim(), env.GIT_COMMIT.substring(0,6)])
...
def javaBindingsWindows(String packageName, List environment) {
  withEnv(environment) {
    currentPath = sh(script: "pwd", returnStdout: true).trim()
    // TODO 30.10.18 @bakhtin Replace static definition of TOOLCHAIN_FILE path with something more configurable
    build.cmakeConfigure('build', "-DCMAKE_BUILD_TYPE=Release -DSWIG_JAVA=ON -DSWIG_JAVA_PKG=${packageName} \
    ...
    build.cmakeBuildWindows('build', '--target irohajava')
    artifactsPath = sprintf('%1$s/java-bindings-%2$s-%3$s-%4$s-%5$s.zip',
      [currentPath, buildType, os, sh(script: 'date "+%Y%m%d"', returnStdout: true).trim(), env.GIT_COMMIT.substring(0,6)])
    // TODO 29.05.18 @bakhtin Java tests never finishes on Windows Server 2016. IR-1380
    sh "pushd build/bindings; \
...
def pythonBindingsLinux(PythonVersion pythonVersion, int parallelism, List environment) {
  withEnv(environment) {
    artifactsPath = sprintf('%1$s/python-bindings-%2$s-%3$s-%4$s-%5$s-%6$s.zip',
      [currentPath, pythonVersion.toString(), 'Release', 'Linux',
      sh(script: 'date "+%Y%m%d"', returnStdout: true).trim(), env.GIT_COMMIT.substring(0,6)])
  }
  currentPath = sh(script: "pwd", returnStdout: true).trim()
  // do not use preinstalled libed25519
  sh "rm -rf /usr/local/include/ed25519*; \
    unlink /usr/local/lib/libed25519.so; \
    rm -f /usr/local/lib/libed25519.so.1.2.2"
  if (pythonVersion.toString() == 'python2') {
    build.cmakeConfigure('build', "-DCMAKE_BUILD_TYPE=Release -DSWIG_PYTHON=ON -DSUPPORT_PYTHON2=ON")
  }
  else if(pythonVersion.toString() == 'python3') {
    build.cmakeConfigure('build', "-DCMAKE_BUILD_TYPE=Release -DSWIG_PYTHON=ON -DSUPPORT_PYTHON2=OFF")
  }
  build.cmakeBuild('build', "--target irohapy", parallelism)
  ...
  """
  sh """
    ${pythonVersion.toString()} -m grpc_tools.protoc --proto_path=shared_model/schema --python_out=build/bindings \
...
def pythonBindingsWindows(PythonVersion pythonVersion, int parallelism, List environment) {
  withEnv(environment) {
    artifactsPath = sprintf('%1$s/python-bindings-%2$s-%3$s-%4$s-%5$s-%6$s.zip',
      [currentPath, pythonVersion.toString(), 'Release', 'Linux',
      sh(script: 'date "+%Y%m%d"', returnStdout: true).trim(), env.GIT_COMMIT.substring(0,6)])
  ...
  build.cmakeBuildWindows('build', "--target irohapy")
  sh "cmake --build build --target python_tests"
  sh "cd build; ctest -R python --output-on-failure"
  sh """
    protoc --proto_path=shared_model/schema \
...
def doAndroidBindings(int abiVersion, int parallelism, List environment) {
  withEnv(environment) {
    currentPath = sh(script: "pwd", returnStdout: true).trim()
    artifactsPath = sprintf('%1$s/android-bindings-%2$s-%3$s-%4$s-%5$s-%6$s.zip',
    [currentPath, "\$PLATFORM", abiVersion, "\$BUILD_TYPE_A",
    sh(script: 'date "+%Y%m%d"', returnStdout: true).trim(), env.GIT_COMMIT.substring(0,6)])
    sh """
    ...
    sh """
      ...
      """
    sh "cmake --build /iroha/shared_model/build --target irohajava -- -j${parallelism}"
