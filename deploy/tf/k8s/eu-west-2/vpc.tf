...
resource "aws_internet_gateway" "k8s_caliper_ci_eu_west_2" {
  vpc_id = "${aws_vpc.k8s_caliper_ci_vpc.id}"
  tags {
...
resource "aws_vpc_peering_connection_accepter" "eu_west_1_eu_west_2" {
  vpc_peering_connection_id = "${var.a_b_peering_id}"
  auto_accept = true
  tags = {
...
resource "aws_vpc_peering_connection" "eu_west_2_eu_west_3" {
  vpc_id = "${aws_vpc.k8s_caliper_ci_vpc.id}"
...
resource "aws_route_table" "k8s_caliper_ci_r2" {
  vpc_id = "${aws_vpc.k8s_caliper_ci_vpc.id}"
  route {
    cidr_block = "${var.a_vpc_cidr_block}"
    vpc_peering_connection_id = "${aws_vpc_peering_connection_accepter.eu_west_1_eu_west_2.id}"
  }
  route {
    cidr_block = "${var.c_vpc_cidr_block}"
    vpc_peering_connection_id = "${aws_vpc_peering_connection.eu_west_2_eu_west_3.id}"
  }
  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = "${aws_internet_gateway.k8s_caliper_ci_eu_west_2.id}"
  }
}

resource "aws_main_route_table_association" "vpc_main" {
  vpc_id         = "${aws_vpc.k8s_caliper_ci_vpc.id}"
  route_table_id = "${aws_route_table.k8s_caliper_ci_r2.id}"
}
