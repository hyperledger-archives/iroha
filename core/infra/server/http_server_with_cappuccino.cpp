...
json responseError(std::string message) {
  return json({{"message", std::move(message)}, {"status", 400}});
}

enum class RequestType { Int, Str, Bool, Float };
...
std::string Torii(std::unique_ptr<Sumeragi::Stub> stub_,
...
void server() {
  ...
  for (const auto& arg : params) argv.push_back((char*)arg.data());
  argv.push_back(nullptr);
  Cappuccino::Cappuccino(argv.size() - 1, argv.data());

  Cappuccino::route<Cappuccino::Method::POST>(
      "/account/register", [](std::shared_ptr<Request> request) -> Response {
        auto res = Response(request);
        auto data = request->json();
        std::string uuid =
            hash::sha3_256_hex(data["publicKey"].get<std::string>());
        ...
        Torii(Sumeragi::NewStub(grpc::CreateChannel(
                  ::peer::myself::getIp() + ":" +
        ...
        res.json(
            json({{"status", 200}, {"message", "successful"}, {"uuid", uuid}}));

        return res;
      });

  Cappuccino::route<Cappuccino::Method::GET>(
      "/account", [](std::shared_ptr<Request> request) -> Response {
  ...
  Cappuccino::route<Cappuccino::Method::POST>(
      "/asset/operation", [](std::shared_ptr<Request> request) -> Response {
        auto res = Response(request);
        auto data = request->json();
        if (!data.empty()) {
        }
        res.json(json({{"status", 200}, {"message", "Ok"}}));
        return res;
      });

  Cappuccino::route<Cappuccino::Method::GET>(
      "/history/transaction", [](std::shared_ptr<Request> request) -> Response {
        std::string uuid = request->params("uuid");
        auto res = Response(request);
        auto tx_json = json::array();

        res.json(json({{"status", 200}, {"history", tx_json}}));
        return res;
      });
