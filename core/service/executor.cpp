...
void transfer(const Transaction& tx) {
  ...
  if (author != tx.asset().value().end() &&
    ...
    auto authorAsset =
        repository::asset::find((*author).second.valuestring(), assetName);
    auto receiverAsset = repository::asset::find(receiver, assetName);

    if (!senderAsset.name().empty() && !authorAsset.name().empty() &&
      ...
      if (senderValue != senderAsset.value().end() &&
        ...
        if ((*senderValue).second.valueint() >= (*value).second.valueint()) {
          (*senderAsset.mutable_value())["value"].set_valueint(
              (google::protobuf::int64)((*senderValue).second.valueint() -
                                        (*value).second.valueint()));
          (*receiverAsset.mutable_value())["value"].set_valueint(
              (google::protobuf::int64)(
                  (*receiverValue).second.valueint() +
                  ((*value).second.valueint() *
                   (1 - (*percent).second.valuedouble()))));
          (*authorAsset.mutable_value())["value"].set_valueint(
              (google::protobuf::int64)((*authorValue).second.valueint() +
                                        (*value).second.valueint() *
                                            (*percent).second.valuedouble()));
          repository::asset::update(sender, assetName, senderAsset);
          repository::asset::update(receiver, assetName, receiverAsset);

          repository::asset::update((*author).second.valuestring(), assetName,
                                    authorAsset);
        } else {
        ...
        logger::error("executor")
            << "sender or receiver's asset does not contain value";
      }
    } else {
      logger::error("executor")
          << "sender or receiver or author does not have asset " << assetName;
    }
  } else {
    logger::error("executor")
        << "Asset does not have author or percent or value";
  }
}
};
...
void transfer(const Transaction& tx) {
  ...
  if (targetName != tx.asset().value().end() &&
    ...
    auto senderAsset = repository::asset::find(sender, assetName);
    auto receiverAsset = repository::asset::find(receiver, assetName);
    if (!senderAsset.name().empty() && !receiverAsset.name().empty()) {
      auto senderHasNum =
          senderAsset.value().find((*targetName).second.valuestring());
      auto receiverHasNum =
          receiverAsset.value().find((*targetName).second.valuestring());
      if (senderHasNum != senderAsset.value().end() &&
          receiverHasNum != receiverAsset.value().end()) {
        (*senderAsset.mutable_value())[(*targetName).second.valuestring()]
            .set_valueint((*senderHasNum).second.valueint() -
                          (*value).second.valueint());
        (*receiverAsset.mutable_value())[(*targetName).second.valuestring()]
            .set_valueint((*receiverHasNum).second.valueint() +
                          (*value).second.valueint());

        repository::asset::update(sender, assetName, senderAsset);
        repository::asset::update(receiver, assetName, receiverAsset);
      } else {
        logger::error("executor") << "sender or receiver does not have target "
                                  << (*targetName).second.valuestring();
      }
    } else {
      logger::error("executor") << "sender or receiver does not have asset "
                                << assetName;
    }
  } else {
...
void transfer(const Transaction& tx) {
  if (tx.has_asset()) {
    ...
    if (type != tx.asset().value().end() &&
        (*type).second.valuestring() == "tax") {
      logger::info("executor") << "tx type is tax ";
      tax::transfer(tx);
    } else if (type != tx.asset().value().end() &&
               (*type).second.valuestring() == "multi_message") {
