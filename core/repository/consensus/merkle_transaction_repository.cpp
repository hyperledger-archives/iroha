...
std::string calculateNewRootHash(
    const ConsensusEvent& event,
    std::vector<std::tuple<std::string, std::string>>& batchCommit) {
  std::string lastInsertion =
      repository::world_state_repository::find("last_insertion");

  if (lastInsertion.empty()) {
    // Note: there is no need to update the tree's DB here, because there is
    // only one transaction--the current!
    return hash(event.transaction());
  }

  std::string parent =
      repository::world_state_repository::find(lastInsertion + "_parent");
  std::string leftChild =
      repository::world_state_repository::find(parent + "_leftChild");
  std::string rightChild =
      repository::world_state_repository::find(parent + "_rightChild");
  ...
  if (rightChild.empty()) {
    ...
    if (!batchCommit
             .empty()) {  // TODO: this may not be the best comparison to use
    ...
    /*
    // Propagate up the tree to the root
    // That code shouldn't even be used right now because we are moving all the
    Merkle root code to ametsuchi.
    while (!parent.empty()) {

        if (!batchCommit.empty()) { // TODO: this may not be the best comparison
    to use
