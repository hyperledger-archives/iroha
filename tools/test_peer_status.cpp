...
void wait_to_get_success_hash(int& flag, string data) {
  ...
  for (;;) {
    ...
    if (reshash.substr(0, must_match_zero) == string("0", must_match_zero))
      break;
    //                string reshash = std::to_string( mt() ) + prehash;
    //                if( reshash.substr(1, must_match_zero ) == string("0",
    //                must_match_zero ) ) break;
  }
...
double test_make_hash(int argc, char* argv[]) {
  ...
  try {
    for (int i = 0; i < all_parallel_calc; i++) {
      thes.push_back(std::thread(wait_to_get_success_hash, std::ref(flags[i]),
                                 std::to_string(i)));
    }
    ...
    for (int i = 0; i < all_parallel_calc; i++) thes[i].detach();
...
int main(int argc, char* argv[]) {
  double elaplsed_time =
      tools::make_test_peer_status::test_make_hash(argc, argv);


  std::cout << "============" << std::endl;
  std::cout << "output: " << tools::make_test_peer_status::filename
            << std::endl;
