...
bool resultToBool(WsvQueryResult<bool> result) {
  return result.match([](iroha::expected::Value<bool> &v) { return v.value; },
                      [](iroha::expected::Error<iroha::ametsuchi::WsvError> e) {
                        return false;
                      });
}
...
bool hasQueryPermission(const std::string &creator,
  ...
  auto has_permission = [&](const std::set<std::string> &perms) {
    // 1. Creator has grant permission from other user
    return (creator != target_account
            and resultToBool(wsv_query.hasAccountGrantablePermission(
        ...
        (
               ...
               (creator == target_account
                and accountHasPermission(perms,
                                         indiv_permission_id))
               or  // 3. Creator has global permission to get any account
               (accountHasPermission(perms,
                                     all_permission_id))
               or  // 4. Creator has domain permission
               (getDomainFromName(creator) == getDomainFromName(target_account)
                and accountHasPermission(perms, domain_permission_id)));
  };
  return perms_set.match(
      [&has_permission](iroha::expected::Value<std::set<std::string>> &v) {
        return has_permission(v.value);
      },
      [](iroha::expected::Error<WsvError> &e) { return false; });
}

bool QueryProcessingFactory::validate(const model::GetAssetInfo &query) {
  // TODO: check signatures
  return resultToBool(checkAccountRolePermission(
...
bool QueryProcessingFactory::validate(const model::GetTransactions &query) {
  ...
  return resultToBool(checkAccountRolePermission(
      ...
      or resultToBool(checkAccountRolePermission(
...
std::shared_ptr<QueryResponse> QueryProcessingFactory::executeGetAssetInfo(
  ...
  return ast.match(
      [&query](const iroha::expected::Value<iroha::model::Asset> &v)
          -> std::shared_ptr<QueryResponse> {
        AssetResponse response;
        response.asset = std::move(v.value);
        response.query_hash = iroha::hash(query);
        return std::make_shared<AssetResponse>(response);
      },
      [&query](const iroha::expected::Error<WsvError> &e)
          -> std::shared_ptr<QueryResponse> {
...
std::shared_ptr<QueryResponse> QueryProcessingFactory::executeGetRoles(
  ...
  return roles.match(
      [&query](const iroha::expected::Value<std::vector<std::string>> &v)
          -> std::shared_ptr<QueryResponse> {
        RolesResponse response;
        response.query_hash = iroha::hash(query);
        response.roles = std::move(v.value);
...
QueryProcessingFactory::executeGetRolePermissions(
  ...
  return perm.match(
      [&query](const iroha::expected::Value<std::vector<std::string>> &v)
        ...
        response.query_hash = iroha::hash(query);
        response.role_permissions = std::move(v.value);
...
std::shared_ptr<QueryResponse> QueryProcessingFactory::executeGetAccount(
  ...
  auto get_account_with_roles = [&](const auto &account)
      -> WsvQueryResult<
          std::pair<iroha::model::Account, std::vector<std::string>>> {
    auto roles = _wsvQuery->getAccountRoles(account.account_id);
    return roles.match(
        [&](iroha::expected::Value<std::vector<std::string>> &v)
            -> WsvQueryResult<
                std::pair<iroha::model::Account, std::vector<std::string>>> {
          return iroha::expected::makeValue(std::make_pair(account, v.value));
        },
        [&](iroha::expected::Error<WsvError> &e)
            -> WsvQueryResult<
                std::pair<iroha::model::Account, std::vector<std::string>>> {
  ...
  return (acc | get_account_with_roles)
      .match(
          [&](const iroha::expected::Value<
              std::pair<iroha::model::Account, std::vector<std::string>>> &v)
              -> std::shared_ptr<QueryResponse> {
            AccountResponse response;
            response.account = v.value.first;
            response.roles = v.value.second;
            response.query_hash = iroha::hash(query);
...
std::shared_ptr<QueryResponse> QueryProcessingFactory::executeGetAccountAssets(
  ...
  return acct_asset.match(
      [&query](const iroha::expected::Value<AccountAsset> &v)
          -> std::shared_ptr<QueryResponse> {
        AccountAssetResponse response;
        response.acct_asset = v.value;
        response.query_hash = iroha::hash(query);
...
iroha::model::QueryProcessingFactory::executeGetAccountDetail(
  ...
  return acct_detail.match(
      [&query](const iroha::expected::Value<std::string> &v)
          -> std::shared_ptr<QueryResponse> {
        iroha::model::AccountDetailResponse response;
        response.detail = v.value;
        response.query_hash = iroha::hash(query);
...
std::shared_ptr<QueryResponse> QueryProcessingFactory::executeGetSignatories(
  ...
  return signs.match(
      [&query](const iroha::expected::Value<std::vector<pubkey_t>> &v)
          -> std::shared_ptr<QueryResponse> {
        SignatoriesResponse response;
        response.query_hash = iroha::hash(query);
        response.keys = v.value;
