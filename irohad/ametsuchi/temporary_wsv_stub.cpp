...
namespace iroha {
  ...
  namespace ametsuchi {
    ...
    model::Account TemporaryWsvStub::get_account(ed25519::pubkey_t pub_key) {
      return ametsuchi_.get_account(pub_key);
    }

    model::Asset TemporaryWsvStub::get_asset(std::string asset_full_name) {
    ...
    std::vector<model::Wallet> TemporaryWsvStub::get_account_wallets(
        ed25519::pubkey_t pub_key) {
      return ametsuchi_.get_account_wallets(pub_key);
    }

    std::vector<model::Asset> TemporaryWsvStub::get_domain_assets(
    ...
    iroha::model::Peer TemporaryWsvStub::get_peer(
