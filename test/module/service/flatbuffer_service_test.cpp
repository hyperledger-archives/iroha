...
TEST(FlatbufferServiceTest, toConsensusEvent) {
  ...
  const auto accountBuf = [&] {
    return flatbuffer_service::CreateAccountBuffer("PublicKey", "Alias",
                                                   {"sig1", "sig2", "sig3"}, 1);
  }();
  ...
  /*
  ASSERT_STREQ(txptr->creatorPubKey()->c_str(), "Creator PubKey");
  ASSERT_EQ(txptr->command_type(), ::iroha::Command_AccountAdd);
    const auto revivedAccount =
        static_cast<const ::iroha::AccountAdd*>(txptr->command())
            ->account_nested_root();

    ASSERT_STREQ(revivedAccount->pubKey()->c_str(), "PublicKey");
    ASSERT_STREQ(revivedAccount->alias()->c_str(), "Alias");
    ASSERT_STREQ(revivedAccount->signatories()->Get(0)->c_str(), "sig1");
    ASSERT_STREQ(revivedAccount->signatories()->Get(1)->c_str(), "sig2");
    ASSERT_STREQ(revivedAccount->signatories()->Get(2)->c_str(), "sig3");
    ASSERT_EQ(revivedAccount->useKeys(), 1);
  */
