...
TEST(Hash, sha3_256_empty_text) {
  ...
  ASSERT_STREQ(iroha::crypto::sha3_256_hex("").c_str(), res.c_str());
}
...
TEST(Hash, sha3_512_empty_text) {
  ...
  ASSERT_STREQ(iroha::crypto::sha3_512_hex("").c_str(), res.c_str());
}

TEST(Hash, sha3_256_ASCII_text) {
  std::string res =
      "cb7c96616a2466df29a1edc2979ef5080945f92d1907c08a55b502eba063d638";
  ASSERT_STREQ(iroha::crypto::sha3_256_hex("Is the Order a distributed ledger?").c_str(),
               res.c_str());
}
...
TEST(Hash, sha3_256_JP_text) {
  ...
  ASSERT_STREQ(iroha::crypto::sha3_256_hex("ご注文は分散台帳ですか？").c_str(),
               res.c_str());
}
...
TEST(Hash, sha3_512_ASCII_text) {
  ...
  ASSERT_STREQ(iroha::crypto::sha3_512_hex("Is the Order a distributed ledger?").c_str(),
               res.c_str());
}
...
TEST(Hash, sha3_512_JP_text) {
  ...
  ASSERT_STREQ(iroha::crypto::sha3_512_hex("ご注文は分散台帳ですか？").c_str(),
               res.c_str());
}
...
TEST(Hash, sha3_256_JP_text_LOOP) {
  ...
  for (int i = 0; i < 100000; i++) {
    EXPECT_STREQ(iroha::crypto::sha3_256_hex("ご注文は分散台帳ですか？").c_str(),
                 res.c_str());
  }
}
...
TEST(Hash, sha3_512_JP_text_LOOP) {
  std::string res =
      ...
      "1897b456d1eacc79bed3acccdc816340263cfff82dd83ddd10e193da";
  for (int i = 0; i < 100000; i++) {
    EXPECT_STREQ(iroha::crypto::sha3_512_hex("ご注文は分散台帳ですか？").c_str(),
                 res.c_str());
  }
}
