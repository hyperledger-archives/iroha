...
TEST(PeerServiceMonitor,ActivePeer){
  ...
  ASSERT_TRUE( peer_service::change_state::validation::setActive(pub1,peer_service::State::ACTIVE) );
  ASSERT_TRUE( peer_service::change_state::runtime::setActive(pub1,peer_service::State::ACTIVE,iroha::time::now64()) );

  ASSERT_TRUE( peer_service::change_state::validation::setActive(pub2,peer_service::State::ACTIVE) );
  ASSERT_TRUE( peer_service::change_state::runtime::setActive(pub2,peer_service::State::ACTIVE,iroha::time::now64()) );

  auto ps = peer_service::monitor::getAllPeerList();
  for( auto p : ps ) {
    log.info(p->getIp() + " " + p->getPublicKey() + " " + std::to_string(p->getState()) );
  }
...
TEST(PeerServiceMonitor,ErasePeer){
  ...
  ASSERT_TRUE( peer_service::change_state::validation::setActive(pub1,peer_service::State::PREPARE) );
  ASSERT_TRUE( peer_service::change_state::runtime::setActive(pub1,peer_service::State::PREPARE) );
