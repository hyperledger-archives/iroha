...
def genesis_tx():
    ...
    genesis_commands.extend([
        iroha.command('CreateAsset', asset_name='coin', domain_id='test', precison=2),
...
def account_asset_transactions_query():
    query = iroha.query('GetAccountAssetTransactions', creator_account=alice['id'], account_id=admin['id'],
                        asset_id='coin#test')
    irohalib.IrohaCrypto.sign_query(query, alice['id'])
    return query
