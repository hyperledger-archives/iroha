...
JNIEXPORT jstring JNICALL Java_repository_Repository_accountAdd(
  ...
  const std::vector<std::string> assets =
      convertJavaStringArrayRelease(env, assets_);
  ...
  logger::debug(NameSpaceID + "::accountAdd") << "pubkey: " << publicKey
                                              << " name: " << name;
  //      << " assets: " << convert_string::to_string(assets);

  Api::Account account;

  for (auto asset : assets) account.add_assets(asset);
...
Java_repository_Repository_accountAttach(JNIEnv *env, jclass, jobject mMap_) {
  const auto mMap = convertJavaHashMapValueString(env, mMap_);
  ...
  logger::debug(NameSpaceID + "::accountAttach")
      << "uuid: " << uuid << ", assetUuid: " << assetUuid;
...
JNIEXPORT jobject JNICALL Java_repository_Repository_accountUpdate(
  ...
  logger::debug(NameSpaceID + "::accountUpdate") << " publicKey: " << publicKey
                                                 << ", name: " << name;
  //      << ", assets: " << convert_string::to_string(assets);
  ...
  return JavaMakeBoolean(
      env, (jboolean)repository::account::update(publicKey, account));
}

JNIEXPORT jobject JNICALL
Java_repository_Repository_accountRemove(JNIEnv *env, jclass, jobject mMap_) {
  const auto mMap = convertJavaHashMapValueString(env, mMap_);
...
JNIEXPORT jobject JNICALL
Java_repository_Repository_accountExists(JNIEnv *env, jclass, jobject mMap_) {
  const auto mMap = convertJavaHashMapValueString(env, mMap_);
...
Java_repository_Repository_domainAdd(JNIEnv *env, jclass, jobject params_) {
...
JNIEXPORT jobject JNICALL
Java_repository_Repository_domainUpdate(JNIEnv *env, jclass, jobject params_) {
  const auto params = convertJavaHashMapValueString(env, params_);
...
JNIEXPORT jobject JNICALL
Java_repository_Repository_domainRemove(JNIEnv *env, jclass, jobject params_) {
  const auto params = convertJavaHashMapValueString(env, params_);
...
JNIEXPORT jobject JNICALL
Java_repository_Repository_domainExists(JNIEnv *env, jclass, jobject params_) {
  const auto params = convertJavaHashMapValueString(env, params_);
  ...
  return JavaMakeBoolean(env, (jboolean)repository::domain::exists(
                                  params.find(tag::Uuid)->second));
}
...
JNIEXPORT jstring JNICALL Java_repository_Repository_assetAdd(JNIEnv *env,
                                                              jclass,
                                                              jobject params_,
                                                              jobject value_) {
  const auto params = convertJavaHashMapValueString(env, params_);
  ...
  logger::debug(NameSpaceID + "::assetAdd") << "domainId: " << domain
                                            << ", assetName: " << name;
  ...
  const auto ret = repository::asset::add(publicKey, name, asset);
...
JNIEXPORT jobject JNICALL
Java_repository_Repository_assetRemove(JNIEnv *env, jclass, jobject params_) {
  const auto params = convertJavaHashMapValueString(env, params_);
  ...
  // WIP
  return JavaMakeBoolean(
      env, (jboolean)repository::asset::remove(publicKey, assetName));
}
...
JNIEXPORT jobject JNICALL Java_repository_Repository_assetInfoFindByUuid(
  ...
  logger::debug(NameSpaceID + "::assetInfoFindByUuid") << "publicKey: "
                                                       << publicKey;
...
JNIEXPORT jobject JNICALL Java_repository_Repository_assetValueFindByUuid(
  ...
  logger::debug(NameSpaceID + "::assetValueFindByUuid") << "publicKey: "
                                                        << publicKey;
...
JNIEXPORT jobject JNICALL
Java_repository_Repository_assetExists(JNIEnv *env, jclass, jobject params_) {
  const auto params = convertJavaHashMapValueString(env, params_);
  ...
  return JavaMakeBoolean(
      env, (jboolean)repository::asset::exists(publicKey, assetName));
}
...
JNIEXPORT jobject JNICALL Java_repository_Repository_simpleAssetInfoFindByUuid(
  ...
  logger::debug(NameSpaceID + "::simpleAssetInfoFindByUuid")
      << "simpleAssetInfo[tag::DomainId]: \"value\":"
      << simpleAssetInfo[tag::DomainId] << ", "
      << "simpleAssetInfo[tag::AssetName]: \"value\":"
      << simpleAssetInfo[tag::AssetName] << ", "
 ...
 * Peer
 ***************************************************************************************/
JNIEXPORT jstring JNICALL Java_repository_Repository_peerAdd(JNIEnv *env,
                                                             jclass,
                                                             jobject params_,
                                                             jobject trust_) {
  const auto params = convertJavaHashMapValueString(env, params_);
  ...
  const auto trust =
      convertMapStringToTrust(convertJavaHashMapValueString(env, trust_));

  logger::debug(NameSpaceID + "::peerAdd") << "publicKey: " << publicKey
                                           << ", address: " << address;
...
JNIEXPORT jobject JNICALL Java_repository_Repository_peerUpdate(
  ...
  logger::debug(NameSpaceID + "::peerUpdate")
...
JNIEXPORT jobject JNICALL
Java_repository_Repository_peerRemove(JNIEnv *env, jclass, jobject params_) {
  const auto params = convertJavaHashMapValueString(env, params_);
...
JNIEXPORT jobject JNICALL
Java_repository_Repository_peerExists(JNIEnv *env, jclass, jobject params_) {
  const auto params = convertJavaHashMapValueString(env, params_);
  ...
  return JavaMakeBoolean(
      env, repository::domain::exists(params.find(tag::Uuid)->second));
}